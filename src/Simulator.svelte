<script lang="ts">
  ...
  function nextValue(v0, power, t, slope) {
    ...
    const e_k0 =
      ...
      Math.pow(v0, 2);
    const e_kr0 =
      (0.5 * bikeSettings.wheelsInertia * Math.pow(v0, 2)) /
      Math.pow(bikeSettings.wheelsRadius, 2);
    const e_w = bikeSettings.efficiency * t * power;
    const ascent = -slope * v0 * t;
    const e_u =
      (bikeSettings.bikeWeight + userSettings.riderWeight) * g * ascent;
    const a_r =
      cr * (bikeSettings.bikeWeight + userSettings.riderWeight) * g * t * v0;
    const a_a =
      0.5 * bikeSettings.rho * cx * bikeSettings.area * Math.pow(v0, 3) * t;
    return (
      3.6 *
      Math.pow(
        (2 * (e_k0 + e_kr0 + e_w + e_u - a_a - a_r)) /
          (bikeSettings.bikeWeight +
            userSettings.riderWeight +
            bikeSettings.wheelsInertia /
              Math.pow(bikeSettings.wheelsRadius, 2)),
        1 / 2
      )
    );
